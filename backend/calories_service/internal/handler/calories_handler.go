...
// GET /api/v1/calories/food?user_id=…&date=YYYY-MM-DD
func (h *CaloriesHandler) GetFoodEntries(c *gin.Context) {
	userID := c.Query("user_id")
	date := c.Query("date")
	if userID == "" || date == "" {
		c.JSON(http.StatusBadRequest, gin.H{"error": model.NewError("INVALID_INPUT", "user_id and date are required")})
	...
	entries, err := h.svc.GetFoodEntries(userID, date)
	if err != nil {
		if err == service.ErrUserNotFound {
			c.JSON(http.StatusNotFound, gin.H{"error": model.NewError("USER_NOT_FOUND", err.Error())})
			return
...
// GET /api/v1/calories/workout?user_id=…&date=YYYY-MM-DD
func (h *CaloriesHandler) GetWorkoutEntries(c *gin.Context) {
	userID := c.Query("user_id")
	date := c.Query("date")
	if userID == "" || date == "" {
		c.JSON(http.StatusBadRequest, gin.H{"error": model.NewError("INVALID_INPUT", "user_id and date are required")})
	...
	entries, err := h.svc.GetWorkoutEntries(userID, date)
	if err != nil {
		if err == service.ErrUserNotFound {
			c.JSON(http.StatusNotFound, gin.H{"error": model.NewError("USER_NOT_FOUND", err.Error())})
			return

...
func (s *CaloriesService) LogFood(input *model.FoodInput) error {
	...
	return s.repo.AddFood(input.UserID, input.EntryDate, input.Name, input.Calories, input.Proteins, input.Fats, input.Carbs, input.MealType)
}
...
func (s *CaloriesService) LogWorkout(input *model.WorkoutInput) error {
	...
	return s.repo.AddWorkout(input.UserID, input.EntryDate, input.Name, input.DurationMinutes, input.CaloriesBurned)
}
...
func (s *CaloriesService) GetFoodEntries(userID, date string) ([]repository.FoodEntry, error) {
	if ok, err := s.profileExists(userID); err != nil {
		return nil, err
	} else if !ok {
		return nil, ErrUserNotFound
	}
	return s.repo.GetFoodEntries(userID, date)
}

func (s *CaloriesService) GetWorkoutEntries(userID, date string) ([]repository.WorkoutEntry, error) {
	if ok, err := s.profileExists(userID); err != nil {
		return nil, err
	} else if !ok {
		return nil, ErrUserNotFound
	}
	return s.repo.GetWorkoutEntries(userID, date)
}
